...
// ============================================================================
// GetDriverLocation Tests
// ============================================================================

func TestGetDriverLocation_Success(t *testing.T) {
	handler, _, _, redisMock := setupTestHandler(t)
	...
	assert.Equal(t, locationData, w.Body.String())
}

func TestGetDriverLocation_EmptyDriverID(t *testing.T) {
	handler, _, _, _ := setupTestHandler(t)

	c, w := setupTestContext("GET", "/api/v1/drivers//location", nil)
	c.Params = gin.Params{{Key: "driver_id", Value: ""}}
	...
	response := parseResponse(w)
	assert.False(t, response["success"].(bool))
	assert.Contains(t, response["error"].(map[string]interface{})["message"], "driver_id is required")
}
...
func TestGetDriverLocation_NotFound(t *testing.T) {
	...
	response := parseResponse(w)
	assert.False(t, response["success"].(bool))
	assert.Contains(t, response["error"].(map[string]interface{})["message"], "Driver location not found")
}

func TestGetDriverLocation_RedisError(t *testing.T) {
	handler, _, _, redisMock := setupTestHandler(t)
